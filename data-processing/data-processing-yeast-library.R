...
dat_yeast_library <- cycle1

# free_energy_yeast_library <- read_csv("data/Created/kim_min_free_energy_yeast_library.csv")
# # free_energy_yeast_library <- free_energy_yeast_library[1:12472,]

# dat_yeast_library <- dat_yeast_library %>%
#   bind_cols(free_energy_yeast_library)
...
dat_yeast_library %>% map_df(~ sum(is.na(.x)))
...
# dat_melting_yeast_library <- dat_yeast_library %>%
#   group_by(1:n()) %>%
...
sequence_1_df_yeast_library <- dat_yeast_library %>%
  pull(x50mer) %>%
  sequence_df()

sequence_2_df_yeast_library <- dat_yeast_library %>%
  pull(x50mer) %>%
  ...
  map(unlist) %>%
  unlist() %>%
  {matrix(., nrow = nrow(dat_yeast_library), byrow = TRUE)} %>%
  data.frame(stringsAsFactors = FALSE)

sequence_3_df_yeast_library <- dat_yeast_library %>%
  pull(x50mer) %>%
  map(~unlist(strsplit(.x, ""))) %>%
...
sequence_4_df_yeast_library <- dat_yeast_library %>%
  pull(x50mer) %>%
  map(~unlist(strsplit(.x, ""))) %>%
...
sequence_5_df_yeast_library <- dat_yeast_library %>%
  pull(x50mer) %>%
  map(~unlist(strsplit(.x, ""))) %>%
...
for(i in 3:49) {
  assign(paste0("interaction_", i, "_df_yeast_library"), dat_yeast_library %>%
           pull(x50mer) %>%
           map(~unlist(strsplit(.x, ""))) %>%
...
for(i in 3:47) {
  assign(paste0("di_interaction_", i, "_df_yeast_library"), dat_yeast_library %>%
           pull(x50mer) %>%
           map(~unlist(strsplit(.x, ""))) %>%
...
sequence_1_factor_yeast_library <- sequence_1_df_yeast_library %>%
  map_df(~ factor(.x, levels = c("A", "C", "G", "T"))) %>%
  as.data.frame() %>%
  rename_all(~paste0(.x, "mono"))

sequence_2_factor_yeast_library <- sequence_2_df_yeast_library %>%
  map_df(~ factor(.x, levels = dinucleotides)) %>%
  as.data.frame() %>%
  rename_all(~paste0(.x, "di")) %>%
  select(-X50di)

sequence_3_factor_yeast_library <- sequence_3_df_yeast_library %>%
  map_df(~ factor(.x, levels = trinucleotides)) %>%
  as.data.frame() %>%
  rename_all(~ paste0(.x, "tri")) %>%
  select(-X49tri, -X50tri)

sequence_4_factor_yeast_library <- sequence_4_df_yeast_library %>%
  map_df(~ factor(.x, levels = tetranucleotides)) %>%
  as.data.frame() %>%
  rename_all(~ paste0(.x, "tetra")) %>%
  select(-X48tetra, -X49tetra, -X50tetra)

sequence_5_factor_yeast_library <- sequence_5_df_yeast_library %>%
  map_df(~ factor(.x, levels = pentanucleotides)) %>%
  as.data.frame() %>%
  rename_all(~ paste0(.x, "penta")) %>%
  select(-X47penta, -X48penta, -X49penta, -X50penta)

factor_seq_yeast_library <- sequence_1_factor_yeast_library %>%
  bind_cols(sequence_2_factor_yeast_library, sequence_3_factor_yeast_library,
            sequence_4_factor_yeast_library, sequence_5_factor_yeast_library)

for(i in 3:49) {
  assign(paste0("interaction_", i, "_factor_yeast_library"), get(paste0("interaction_", i, "_df_yeast_library")) %>%
           map_df(~ factor(.x, levels = dinucleotides)) %>%
           as.data.frame() %>%
           rename_all(~paste0(.x, "int", i)))
}
interaction_49_factor_yeast_library <- interaction_49_factor_yeast_library %>%
  rename(X1int49 = .int49)

factor_int_yeast_library <- interaction_3_factor_yeast_library
for(i in 4:49) {
  factor_int_yeast_library <- factor_int_yeast_library %>%
    bind_cols(get(paste0("interaction_", i, "_factor_yeast_library")))
}


for(i in 3:47) {
  assign(paste0("di_interaction_", i, "_factor_yeast_library"), 
         get(paste0("di_interaction_", i, "_df_yeast_library")) %>%
           map_df(~ factor(.x, levels = tetranucleotides)) %>%
           as.data.frame() %>%
           rename_all(~paste0(.x, "di_int", i)))
}
di_interaction_47_factor_yeast_library <- di_interaction_47_factor_yeast_library %>%
  rename(X1di_int47 = .di_int47)

factor_di_int_yeast_library <- di_interaction_3_factor_yeast_library
for(i in 4:47) {
  factor_di_int_yeast_library <- factor_di_int_yeast_library %>%
    bind_cols(get(paste0("di_interaction_", i, "_factor_yeast_library")))
}

library(stringi)

nc_1_yeast_library <- nucleotides %>%
  map(~ str_count(dat_yeast_library$x50mer, .x)) %>%
  bind_cols()

nc_2_yeast_library<- dinucleotides %>%
  map(~ str_count(dat_yeast_library$x50mer, paste0("(?=",.x,")"))) %>%
  bind_cols()

nc_3_yeast_library <- trinucleotides %>%
  map(~ str_count(dat_yeast_library$x50mer, paste0("(?=",.x,")"))) %>%
  bind_cols()

colnames(nc_1_yeast_library) <- nucleotides
colnames(nc_2_yeast_library) <- dinucleotides
colnames(nc_3_yeast_library) <- trinucleotides
...
#GC count
gc_count_yeast_library <- nc_1_yeast_library %>%
  transmute(gc_count = G + C)

dat_seq_yeast_library <- factor_seq_yeast_library %>%
  bind_cols(nc_1_yeast_library,
            nc_2_yeast_library,
            nc_3_yeast_library,
            gc_count_yeast_library,
            dat_yeast_library)

# dat_yeast_library <- dat_yeast_library %>%
#   bind_cols(factor_seq_yeast_library,
#             factor_int_yeast_library,
#             factor_di_int_yeast_library,
#             nc_1_yeast_library,
#             nc_2_yeast_library,
#             nc_3_yeast_library, 
#             gc_count_yeast_library,
#             dat_melting_yeast_library)
...
train_indices_yeast_library = sample(1:nrow(dat_yeast_library), 
                                    nrow(dat_yeast_library)*.9, replace=FALSE)

dat_train_yeast_library = dat_yeast_library[train_indices_yeast_library,]
dat_test_yeast_library = dat_yeast_library[-train_indices_yeast_library,]
dat_seq_yeast_library = dat_seq_yeast_library[train_indices_yeast_library,]
dat_yeast_library = dat_train_yeast_library

saveRDS(dat_yeast_library, "data/Created/processed_yeast_library.rds")
saveRDS(dat_test_yeast_library, "data/Created/processed_test_yeast_library.rds")
saveRDS(dat_seq_yeast_library, "data/Created/processed_seq_yeast_library.rds")
