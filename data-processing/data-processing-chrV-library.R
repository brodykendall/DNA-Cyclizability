...
dat_chrV_library <- cycle6

# free_energy_chrV_library <- read_csv("data/Created/kim_min_free_energy_chrV_library.csv")
# # free_energy_chrV_library <- free_energy_chrV_library[1:12472,]

# dat_chrV_library <- dat_chrV_library %>%
#   bind_cols(free_energy_chrV_library)
...
dat_chrV_library %>% map_df(~ sum(is.na(.x)))
...
# dat_melting_chrV_library <- dat_chrV_library %>%
#   group_by(1:n()) %>%
...
sequence_1_df_chrV_library <- dat_chrV_library %>%
  pull(x50mer) %>%
  sequence_df()

sequence_2_df_chrV_library <- dat_chrV_library %>%
  pull(x50mer) %>%
  ...
  map(unlist) %>%
  unlist() %>%
  {matrix(., nrow = nrow(dat_chrV_library), byrow = TRUE)} %>%
  data.frame(stringsAsFactors = FALSE)

sequence_3_df_chrV_library <- dat_chrV_library %>%
  pull(x50mer) %>%
  map(~unlist(strsplit(.x, ""))) %>%
...
sequence_4_df_chrV_library <- dat_chrV_library %>%
  pull(x50mer) %>%
  map(~unlist(strsplit(.x, ""))) %>%
...
sequence_5_df_chrV_library <- dat_chrV_library %>%
  pull(x50mer) %>%
  map(~unlist(strsplit(.x, ""))) %>%
...
for(i in 3:49) {
  assign(paste0("interaction_", i, "_df_chrV_library"), dat_chrV_library %>%
           pull(x50mer) %>%
           map(~unlist(strsplit(.x, ""))) %>%
...
for(i in 3:47) {
  assign(paste0("di_interaction_", i, "_df_chrV_library"), dat_chrV_library %>%
           pull(x50mer) %>%
           map(~unlist(strsplit(.x, ""))) %>%
...
sequence_1_factor_chrV_library <- sequence_1_df_chrV_library %>%
  map_df(~ factor(.x, levels = c("A", "C", "G", "T"))) %>%
  as.data.frame() %>%
  rename_all(~paste0(.x, "mono"))

sequence_2_factor_chrV_library <- sequence_2_df_chrV_library %>%
  map_df(~ factor(.x, levels = dinucleotides)) %>%
  as.data.frame() %>%
  rename_all(~paste0(.x, "di")) %>%
  select(-X50di)

sequence_3_factor_chrV_library <- sequence_3_df_chrV_library %>%
  map_df(~ factor(.x, levels = trinucleotides)) %>%
  as.data.frame() %>%
  rename_all(~ paste0(.x, "tri")) %>%
  select(-X49tri, -X50tri)

sequence_4_factor_chrV_library <- sequence_4_df_chrV_library %>%
  map_df(~ factor(.x, levels = tetranucleotides)) %>%
  as.data.frame() %>%
  rename_all(~ paste0(.x, "tetra")) %>%
  select(-X48tetra, -X49tetra, -X50tetra)

sequence_5_factor_chrV_library <- sequence_5_df_chrV_library %>%
  map_df(~ factor(.x, levels = pentanucleotides)) %>%
  as.data.frame() %>%
  rename_all(~ paste0(.x, "penta")) %>%
  select(-X47penta, -X48penta, -X49penta, -X50penta)

factor_seq_chrV_library <- sequence_1_factor_chrV_library %>%
  bind_cols(sequence_2_factor_chrV_library, sequence_3_factor_chrV_library,
            sequence_4_factor_chrV_library, sequence_5_factor_chrV_library)

for(i in 3:49) {
  assign(paste0("interaction_", i, "_factor_chrV_library"), get(paste0("interaction_", i, "_df_chrV_library")) %>%
           map_df(~ factor(.x, levels = dinucleotides)) %>%
           as.data.frame() %>%
           rename_all(~paste0(.x, "int", i)))
}
interaction_49_factor_chrV_library <- interaction_49_factor_chrV_library %>%
  rename(X1int49 = .int49)

factor_int_chrV_library <- interaction_3_factor_chrV_library
for(i in 4:49) {
  factor_int_chrV_library <- factor_int_chrV_library %>%
    bind_cols(get(paste0("interaction_", i, "_factor_chrV_library")))
}


for(i in 3:47) {
  assign(paste0("di_interaction_", i, "_factor_chrV_library"), 
         get(paste0("di_interaction_", i, "_df_chrV_library")) %>%
           map_df(~ factor(.x, levels = tetranucleotides)) %>%
           as.data.frame() %>%
           rename_all(~paste0(.x, "di_int", i)))
}
di_interaction_47_factor_chrV_library <- di_interaction_47_factor_chrV_library %>%
  rename(X1di_int47 = .di_int47)

factor_di_int_chrV_library <- di_interaction_3_factor_chrV_library
for(i in 4:47) {
  factor_di_int_chrV_library <- factor_di_int_chrV_library %>%
    bind_cols(get(paste0("di_interaction_", i, "_factor_chrV_library")))
}

library(stringi)

nc_1_chrV_library <- nucleotides %>%
  map(~ str_count(dat_chrV_library$x50mer, .x)) %>%
  bind_cols()

nc_2_chrV_library<- dinucleotides %>%
  map(~ str_count(dat_chrV_library$x50mer, paste0("(?=",.x,")"))) %>%
  bind_cols()

nc_3_chrV_library <- trinucleotides %>%
  map(~ str_count(dat_chrV_library$x50mer, paste0("(?=",.x,")"))) %>%
  bind_cols()

colnames(nc_1_chrV_library) <- nucleotides
colnames(nc_2_chrV_library) <- dinucleotides
colnames(nc_3_chrV_library) <- trinucleotides
...
#GC count
gc_count_chrV_library <- nc_1_chrV_library %>%
  transmute(gc_count = G + C)

dat_seq_chrV_library <- factor_seq_chrV_library %>%
  bind_cols(nc_1_chrV_library,
            nc_2_chrV_library,
            nc_3_chrV_library,
            gc_count_chrV_library,
            dat_chrV_library)

# dat_chrV_library <- dat_chrV_library %>%
#   bind_cols(factor_seq_chrV_library,
#             factor_int_chrV_library,
#             factor_di_int_chrV_library,
#             nc_1_chrV_library,
#             nc_2_chrV_library,
#             nc_3_chrV_library, 
#             gc_count_chrV_library,
#             dat_melting_chrV_library)
...
train_indices_chrV_library = sample(1:nrow(dat_chrV_library), 
                                      nrow(dat_chrV_library)*.9, replace=FALSE)

dat_train_chrV_library = dat_chrV_library[train_indices_chrV_library,]
dat_test_chrV_library = dat_chrV_library[-train_indices_chrV_library,]
dat_seq_chrV_library = dat_seq_chrV_library[train_indices_chrV_library,]
dat_chrV_library = dat_train_chrV_library

saveRDS(dat_chrV_library, "data/Created/processed_chrV_library.rds")
saveRDS(dat_test_chrV_library, "data/Created/processed_test_chrV_library.rds")
saveRDS(dat_seq_chrV_library, "data/Created/processed_seq_chrV_library.rds")
