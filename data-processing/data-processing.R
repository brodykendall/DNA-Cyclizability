...
free_energy <- read_csv("data/Created/tiling_min_free_energy.csv")
...
dat_melting <- dat %>%
  group_by(1:n()) %>%
  transmute(tm1 = Tm_Wallace(x50mer)$Tm) %>%
  ungroup() %>%
  select(-`1:n()`)
...
train_indices = sample(1:nrow(dat_final), nrow(dat_final)*.9, replace=FALSE)

dat_train = dat_final[train_indices,]
dat_test = dat_final[-train_indices,]
dat_seq = dat_seq[train_indices,]
# dat_final = dat_train

saveRDS(dat_train, "data/Created/processed.rds")
