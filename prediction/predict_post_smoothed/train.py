...
def initialize_model(model_name:str, output_features, hyperparameters, in_features):
    if model_name == "FeedForward":
        net = FeedForward(nodes_per_layer=hyperparameters["nodes_per_layer"], num_layers=hyperparameters["num_layers"], 
                  output_features=output_features, dropout=hyperparameters["dropout"],
                  input_features=in_features)
    return net

def train(train_data, hyperparameters, logging_info, in_features, model_name:str, val_data=None, plot_0_val=False, fold=0):
    ...
    patience = hyperparameters["patience"] if "patience" in hyperparameters.keys() else num_epochs
    ...
    if not os.path.exists(folder_path + "models/fold" + str(fold)):
        os.makedirs(folder_path + "models/fold" + str(fold))
    model_save_path = folder_path + "models/fold" + str(fold) + "/" + file_name

    train_loader = DataLoader(train_data, batch_size=batch_size, shuffle=True)
    if val_data is not None:
        val_loader = DataLoader(val_data, batch_size=1)
    
    net = initialize_model(model_name=model_name, output_features=1, hyperparameters=hyperparameters, in_features=in_features)
