...
# sequence_df_AT <- function(sequence_vector){
#   data.frame(matrix(lapply(unlist(strsplit(sequence_vector, "")),
#                            function(sequence_segment) {
#                              if(("A" %in% sequence_segment) | ("T" %in% sequence_segment)) {
#                                return(1)
#                              }
#                              else {
#                                return(0)
...
contain_AT <- function(sequence_segment)  {
  if(grepl("A", sequence_segment, fixed=TRUE) | grepl("T", sequence_segment, fixed=TRUE)) {
    return(1)
  }
  else {
    return(0)
...
#Similar to splitWithOverlap, butt akes only the endpoints
# used to obtain interaction sequences for nucleotides with distance d=overlap. 
# Use seq.length = overlap+1
splitWithOverlap2 <- function(vec, seg.length, overlap) {
  starts = seq(1, length(vec), by=seg.length-overlap)
  ends   = starts + seg.length - 1
  
  lapply(1:(length(starts)-overlap), function(i) vec[c(starts[i],ends[i])])
}

# used to obtain interaction sequences for dinucleotides with distance d=overlap. 
# Use seq.length = overlap+1
splitWithOverlap3 <- function(vec, seg.length, overlap) {
  starts = seq(1, length(vec), by=seg.length-overlap)
  ends   = starts + seg.length - 1
  
  lapply(1:(length(starts)-overlap-2), function(i) vec[c(starts[i], starts[i+1], ends[i+1], ends[i+2])])
}
...
splitWithOverlap2(sequence_vector, 5, 4)
...
splitWithOverlap3(sequence_vector, 5, 4)
